...
* The generated document's Sources line should list only sources actually used for that sheet, including selected English/Hebrew versions.
* Every Sefaria text request should include `return_format=text_only`; keep the existing HTML-stripping fallback for cached old responses and unexpected API output.
...
* The CLI should expose the same feature set with explicit options: `--language-mode`, `--english-version`, and `--hebrew-version`.
  * CLI choices should use the stable internal slugs.
  * CLI defaults should preserve current behavior: `--language-mode bilingual`, `--english-version koren`, `--hebrew-version nikkud`.
...
* Bilingual output should keep the existing column order: Hebrew column first, English column second.
...
* Hebrew divine-name replacement should apply consistently to all Hebrew Tanakh output.
* Divine-name replacement should be exposed as a web checkbox and CLI option, not only as an environment variable.
  ...
  * In English-only output, hide/disable the web checkbox and ignore the generator option because there is no Hebrew text to transform.
...
* [x] Add generator configuration objects/slugs for language mode, English versions, Hebrew versions, and divine-name replacement.
...
* [x] Add CLI options for language mode, English/Hebrew versions, and divine-name replacement.
